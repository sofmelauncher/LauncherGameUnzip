function Copy-StrictedFilterFileWithDirectoryStructure {
    ...
    process {
        ...
        Foreach ($exclude in $Excludes) {
            Get-ChildItem -Path $Destination -Recurse -File | Where-Object Name -like $exclude | Remove-Item
        }
 
        # search Folder which include file
        $allFolder = Get-ChildItem $Destination -Recurse -Directory
        $containsFile = $allFolder | Where-Object { $_.GetFiles() }
        $containsFile.FullName `
        | % {
            $fileContains = $_
            $result = $allFolder.FullName `
            | Where-Object { $_ -notin $list } `
            | Where-Object {
                $shortPath = $_
        ...
        $folderToKeep = $list | Sort-Object -Unique
 
        # Remove All Empty (none file exist) folders
        Get-ChildItem -Path $Destination -Recurse -Directory | Where-Object fullName -notin $folderToKeep | Remove-Item -Recurse
    }
}
